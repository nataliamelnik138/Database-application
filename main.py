...
def main():
    ...
    request_1 = DBManager.get_companies_and_vacancies_count(params)
    for row in request_1:
        print(f"{row[0]} - {row[1]}")

    print("\nСписок всех вакансий:")
    request_2 = DBManager.get_all_vacancies(params)
    for row in request_2:
        print(row)

    print("\nСредняя зарплата по вакансиям:")
    request_3 = DBManager.get_avg_salary(params)
    print(request_3)

    print("\nСписок всех вакансий, у которых зарплата выше средней по всем вакансиям:")
    request_4 = DBManager.get_vacancies_with_higher_salary(params)
    for row in request_4:
        print(row[0])

    keyword = 'Python разработчик'
    print(f'\nСписок всех вакансий, в названии которых содержится "{keyword}":')
    request_5 = DBManager.get_vacancies_with_keyword(params, keyword)
