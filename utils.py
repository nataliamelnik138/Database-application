...
def get_information_about_employers(list_employers_id: list) -> list:
    """
    Получает данные о работодателях с сайта hh.ru
    :param list_employers_id: список id работодателей, информация о которых будет получаться
    :return: список, содержащий информацию о работодателях
    """
    employers = []
...
def get_vacancies(employer_id: int) -> list:
    """
    Получает данные о вакансиях работодателя по его id с сайта hh.ru
    :param employer_id: id работодателя, вакансии которого будут получены
...
def create_database(params, db_name) -> None:
    ...
    cur.close()  # Закрываем курсор перед закрытием соединения
    conn.close()
...
def insert_data(cur, employers: list[dict], vacancies: dict) -> None:
    """Добавляет данные из списков в таблицы employers и vacancies."""
    key = 1
